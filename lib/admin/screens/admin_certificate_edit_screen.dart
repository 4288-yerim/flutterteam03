...
class AdminCertificateEditScreen extends StatefulWidget {
  const AdminCertificateEditScreen({
    super.key,
  ...
  @override
  State<AdminCertificateEditScreen> createState() =>
      _AdminCertificateEditScreenState();
}

class _AdminCertificateEditScreenState
    extends State<AdminCertificateEditScreen> {
  ...
  List<Widget> _informationSections() => [
    ...
    _section('응시 수수료', [
      _field(_feeRound1, '필기/통합 수수료', number: true),
      const SizedBox(height: 10),
      _field(_feeRound2, '실기/면접 수수료', number: true),
      const SizedBox(height: 14),
      _linkFields(_feeLinks),
    ]),
    const SizedBox(height: 16),
    _section('시험 경향', [
      _field(_trends, '내용', lines: 5),
      const SizedBox(height: 14),
      _linkFields(_trendLinks),
    ]),
    const SizedBox(height: 16),
    _section('취득 방법', [
      _field(_obtain, '내용', lines: 5),
      const SizedBox(height: 14),
      _linkFields(_obtainLinks),
    ]),
  ];
      ...
      for (final entry in _schedules.asMap().entries)
        ListTile(
          ...
          trailing: IconButton(
            ...
            icon: Icon(Icons.close_rounded, color: context.colors.incorrect),
          ),
        ),
      const SizedBox(height: 16),
      AppButton(
  ...
  Widget _linkFields(List<_LinkEditor> links) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
        ...
        Row(
          children: [
            Expanded(
              child: Column(
                children: [
                  _field(links[index].label, '링크 버튼 문구'),
                  const SizedBox(height: 8),
                  _field(links[index].url, '링크 주소'),
                ],
              ),
            ),
            IconButton(
  ...
  List<_LinkEditor> _makeEditors(Map<String, dynamic>? detail) {
    final result = (detail?['links'] as List? ?? const [])
        .map(CertificateContentLink.fromMap)
        .map((link) => _LinkEditor(link.label, link.url))
        .toList();
  ...
  List<Map<String, String>> _maps(Iterable<_LinkEditor> links) => links
      .where((link) => link.label.text.trim().isNotEmpty)
      .map(
        (link) => {
  ...
  Future<void> _save() async {
    ...
    if (!_validLinks([
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('링크 문구와 올바른 http(s) 주소를 모두 입력해주세요.')),
      );
      return;
    }
    ...
    details['overview'] = {
      ...?details['overview'],
      'contents': _overview.text.trim(),
    };
    details['examFee'] = {
      'feeRound1': _feeRound1.text.trim(),
      'feeRound2': _feeRound2.text.trim(),
      'links': _maps(_feeLinks),
    };
    details['examTrends'] = {
      'contents': _trends.text,
      'links': _maps(_trendLinks),
    };
    details['howToObtain'] = {
      'contents': _obtain.text,
      'links': _maps(_obtainLinks),
    };
    details['scheduleLinks'] = {'links': _maps(_scheduleLinks)};
    try {
      await _service.saveEditorData(
        AdminCertificateEditorData(
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('저장하지 못했습니다.')));
      }
    } finally {
  ...
  Future<void> _addSchedule() async {
    final draft = await showDialog<AdminCertificateScheduleDraft>(
      context: context,
...
class _ScheduleDialogState extends State<_ScheduleDialog> {
          ...
          if (_errorMessage != null) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: context.colors.incorrect.withValues(alpha: 0.35),
                ),
              ),
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _dateRangeFields({
    ...
    child: Row(
      children: [
        Expanded(child: _dateField(startLabel, startValue, onStartChanged)),
        const Padding(
  ...
  void _submit() {
    ...
    Navigator.pop(
      context,
      AdminCertificateScheduleDraft(
        id: 'admin_${DateTime.now().microsecondsSinceEpoch}',
        title: _title.text.trim(),
        writtenRegistrationStartAt: _writtenRegistrationStart,
        writtenRegistrationEndAt: _writtenRegistrationEnd,
        writtenExamStartAt: _writtenExamStart,
        writtenExamEndAt: _writtenExamEnd,
        writtenPassStartAt: _writtenPassStart,
        writtenPassEndAt: _writtenPassEnd,
        practicalRegistrationStartAt: _practicalRegistrationStart,
        practicalRegistrationEndAt: _practicalRegistrationEnd,
        practicalExamStartAt: _practicalExamStart,
        practicalExamEndAt: _practicalExamEnd,
        practicalPassStartAt: _practicalPassStart,
        practicalPassEndAt: _practicalPassEnd,
        documentSubmitStartAt: _documentSubmitStart,
        documentSubmitEndAt: _documentSubmitEnd,
      ),
    );
  }
}
