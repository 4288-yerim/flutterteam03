...
class _StudyRecordPageState extends State<StudyRecordPage> {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> _getRecordStream(String uid) {
  ...
  DateTime? _getRecordDate(Map<String, dynamic> recordData) {
    ...
    String studyDate = recordData['studyDate']?.toString() ?? '';
  ...
  String _getSubjectName(Map<String, dynamic> recordData) {
    String subject = recordData['subject']?.toString() ?? '';

    if (subject.isEmpty) {
      subject = recordData['studySubject']?.toString() ?? '';
    }
  ...
  DateTime _getPeriodEnd() {
    ...
    if (_selectedPeriod == '오늘') {
      return periodStart.add(Duration(days: 1));
    }

    if (_selectedPeriod == '이번 주') {
      return periodStart.add(Duration(days: 7));
    }

    return DateTime(periodStart.year, periodStart.month + 1, 1);
  }

  bool _isInSelectedPeriod(DateTime recordDate) {
    DateTime periodStart = _getPeriodStart();
    ...
    bool isBeforeEnd = recordDate.isBefore(periodEnd);
  ...
  List<QueryDocumentSnapshot<Map<String, dynamic>>> _filterRecordList(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> allRecordList,
  ) {
    List<QueryDocumentSnapshot<Map<String, dynamic>>> visibleRecordList = [];

    for (int i = 0; i < allRecordList.length; i++) {
      Map<String, dynamic> recordData = allRecordList[i].data();

      DateTime? recordDate = _getRecordDate(recordData);
      ...
      if (_isInSelectedPeriod(recordDate)) {
        visibleRecordList.add(allRecordList[i]);
      }
    }
    ...
    visibleRecordList.sort((a, b) {
      ...
      int aTime = aDate?.millisecondsSinceEpoch ?? 0;

      int bTime = bDate?.millisecondsSinceEpoch ?? 0;
  ...
  List<QueryDocumentSnapshot<Map<String, dynamic>>> _filterRecordListByDate(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> allRecordList,
    DateTime selectedDate,
  ) {
    List<QueryDocumentSnapshot<Map<String, dynamic>>> selectedDateRecordList =
        [];

    for (int i = 0; i < allRecordList.length; i++) {
      DateTime? recordDate = _getRecordDate(allRecordList[i].data());

      if (recordDate == null) {
        continue;
      }

      if (_isSameDate(recordDate, selectedDate)) {
        selectedDateRecordList.add(allRecordList[i]);
      }
    }
  ...
  Widget _buildSummaryCard(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> recordList,
  ) {
    int totalSeconds = 0;
    Set<String> studyDateSet = {};

    for (int i = 0; i < recordList.length; i++) {
      Map<String, dynamic> recordData = recordList[i].data();

      totalSeconds += _getRecordStudySeconds(recordData);

      DateTime? recordDate = _getRecordDate(recordData);

      if (recordDate != null) {
        studyDateSet.add(_formatDateKey(recordDate));
      }
    }
    ...
    if (studyDateSet.isNotEmpty) {
      averageSeconds = totalSeconds ~/ studyDateSet.length;
    }
  ...
  Map<String, int> _createSubjectSecondsMap(
    ...
    for (int i = 0; i < recordList.length; i++) {
      ...
      String subjectName = _getSubjectName(recordData);

      int previousSeconds = subjectSecondsMap[subjectName] ?? 0;
  ...
  Widget _buildSubjectSummary(
    ...
    Map<String, int> subjectSecondsMap = _createSubjectSecondsMap(recordList);

    List<MapEntry<String, int>> subjectList = subjectSecondsMap.entries
        .toList();
    ...
    if (subjectList.isEmpty) {
      return SizedBox(
        ...
        child: AppEmptyView(
          ...
          description: '과목을 선택해 공부하고 종료하면 이곳에 표시됩니다.',
        ),
      );
    }
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(
                      _formatStudyTime(subjectList[i].value),
                      style: TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.bold,
                        color: context.colors.pinkStart,
                ...
                ClipRRect(
                  ...
                  child: LinearProgressIndicator(
                    minHeight: 7,
                    value: subjectList[i].value / maxSeconds,
                    backgroundColor: context.colors.pinkSoft,
  ...
  Map<String, int> _createDailySecondsMap(
    ...
    for (int i = 0; i < recordList.length; i++) {
      ...
      String dateKey = _formatDateKey(recordDate);

      int previousSeconds = dailySecondsMap[dateKey] ?? 0;
  ...
  Widget _buildDailySummary(
    ...
    Map<String, int> dailySecondsMap = _createDailySecondsMap(recordList);

    List<MapEntry<String, int>> dailyList = dailySecondsMap.entries.toList();
    ...
    if (dailyList.isEmpty) {
      return SizedBox(
        ...
        child: AppEmptyView(
          ...
          description: '공부시간을 저장하면 날짜별 기록이 표시됩니다.',
        ),
      );
    }
              ...
              children: [
                ...
                Expanded(
                  child: ClipRRect(
                    ...
                    child: LinearProgressIndicator(
                      minHeight: 8,
                      value: dailyList[i].value / maxSeconds,
                ...
                SizedBox(
                  width: 78,
                  child: Text(
                    _formatStudyTime(dailyList[i].value),
    ...
    if (recordList.isEmpty) {
      return SizedBox(
        ...
        child: AppEmptyView(
          ...
          description: '공부를 종료해 기록을 저장하면 이곳에 표시됩니다.',
        ),
      );
    }
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        _getSubjectName(recordList[i].data()),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: TextStyle(
                          fontSize: 14,
                      ...
                      SizedBox(height: 4),
                      Text(
                        _getRecordDate(recordList[i].data()) == null
                            ? ''
                            : showTimeOnly
                            ? _formatSessionTime(
                                _getRecordDate(recordList[i].data())!,
                              )
                            : _formatDisplayDate(
                                _getRecordDate(recordList[i].data())!,
                              ),
                ...
                Text(
                  _formatStudyTime(
                    _getRecordStudySeconds(recordList[i].data()),
                  ),
                  style: TextStyle(
                    fontSize: 12,
                    fontWeight: FontWeight.bold,
  ...
  int _calculateStudyStreak(
    ...
    if (!studyDateSet.contains(_formatDateKey(today))) {
      DateTime yesterday = today.subtract(Duration(days: 1));

      if (!studyDateSet.contains(_formatDateKey(yesterday))) {
        return 0;
      }
    ...
    while (studyDateSet.contains(_formatDateKey(streakDate))) {
      streak++;
  ...
  int _getMaximumDailySeconds(Map<String, int> dailySecondsMap) {
    int maximumSeconds = 0;

    dailySecondsMap.forEach((dateKey, seconds) {
  ...
  Color _getCalendarCellColor(int studySeconds, int maximumSeconds) {
    if (studySeconds <= 0 || maximumSeconds <= 0) {
      return Theme.of(context).colorScheme.surface;
    }

    double ratio = studySeconds / maximumSeconds;

    if (ratio > 1) {
      ratio = 1;
    }

    double opacity = 0.18 + (ratio * 0.68);

    return context.colors.pinkStart.withOpacity(opacity);
  }

  Color _getCalendarTextColor(int studySeconds, int maximumSeconds) {
    if (studySeconds <= 0 || maximumSeconds <= 0) {
  ...
  void _moveCalendarMonth(int monthDifference) {
  ...
  Widget _buildCalendarLegendBox(double opacity) {
    return Container(
      width: 17,
      height: 17,
      decoration: BoxDecoration(
        color: context.colors.pinkStart.withOpacity(opacity),
  ...
  Widget _buildMonthlyCalendar(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> allRecordList,
    List<QueryDocumentSnapshot<Map<String, dynamic>>> monthRecordList,
  ) {
    Map<String, int> dailySecondsMap = _createDailySecondsMap(monthRecordList);

    int maximumSeconds = _getMaximumDailySeconds(dailySecondsMap);

    int streak = _calculateStudyStreak(allRecordList);
    ...
    int leadingEmptyCellCount = firstDay.weekday - 1;

    int usedCellCount = leadingEmptyCellCount + daysInMonth;

    int totalCellCount = ((usedCellCount + 6) ~/ 7) * 7;
        ...
        children: [
              ...
              Expanded(
                child: Column(
                  children: [
                    ...
                    SizedBox(height: 3),
                    Text(
                      streak > 0 ? '현재 $streak일 연속 공부 중' : '연속 공부 기록이 없습니다.',
                      style: TextStyle(
                        fontSize: 11,
                        color: streak > 0
                            ? context.colors.pinkStart
          ...
          GridView.builder(
            ...
            itemBuilder: (context, index) {
              int dayNumber = index - leadingEmptyCellCount + 1;
              ...
              int studySeconds = dailySecondsMap[dateKey] ?? 0;

              bool isSelected = _isSameDate(date, _selectedCalendarDate);

              bool isToday = _isSameDate(date, _getTodayStart());
              ...
              return InkWell(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: _getCalendarCellColor(studySeconds, maximumSeconds),
                    borderRadius: BorderRadius.circular(11),
                    border: Border.all(
                      color: isSelected
                          ? context.colors.pinkStart
  ...
  Widget _buildMonthSelectedDateRecords(
    ...
    List<QueryDocumentSnapshot<Map<String, dynamic>>> selectedDateRecordList =
        _filterRecordListByDate(allRecordList, _selectedCalendarDate);

    int selectedDateTotalSeconds = 0;

    for (int i = 0; i < selectedDateRecordList.length; i++) {
      selectedDateTotalSeconds += _getRecordStudySeconds(
      ...
      children: [
        Row(
          children: [
            ...
            Text(
              _formatStudyTime(selectedDateTotalSeconds),
              style: TextStyle(
                fontSize: 12,
                fontWeight: FontWeight.bold,
                color: context.colors.pinkStart,
        ...
        SizedBox(height: 11),
        _buildSessionList(selectedDateRecordList, showTimeOnly: true),
      ],
    );
  }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 15),
            _buildSummaryCard(visibleRecordList),
            if (_selectedPeriod == '이번 달') ...[
              ...
              SizedBox(height: 11),
              _buildMonthlyCalendar(allRecordList, visibleRecordList),
              SizedBox(height: 24),
              _buildMonthSelectedDateRecords(allRecordList),
              SizedBox(height: 14),
            ],
            SizedBox(height: 24),
            Text(
            ...
            SizedBox(height: 11),
            _buildSubjectSummary(visibleRecordList),
            if (_selectedPeriod != '이번 달') ...[
              SizedBox(height: 14),
              Text(
                _selectedPeriod == '오늘' ? '오늘 공부 기록' : '날짜별 공부시간',
                style: TextStyle(
                  fontSize: 17,
                  fontWeight: FontWeight.bold,
                  color: context.colors.textPrimary,
              ...
              SizedBox(height: 11),
              if (_selectedPeriod == '오늘')
                _buildSessionList(visibleRecordList)
              else
                _buildDailySummary(visibleRecordList),
            ],
          ],
        ),
      ),
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: AppEmptyView(
              message: '공부 기록이 없습니다.',
              description: '과목을 선택해 공부를 시작하고 종료하면 기록이 저장됩니다.',
            ),
          ),
        ],
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppTopBar(title: '공부 기록'),
      body: currentUser == null
          ? AppErrorView(
              message: '로그인 정보를 확인할 수 없습니다.',
              description: '다시 로그인한 뒤 공부 기록을 확인해 주세요.',
            )
          : StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              stream: _getRecordStream(currentUser.uid),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return AppLoadingView(message: '공부 기록을 불러오는 중입니다.');
                }
                ...
                if (snapshot.hasError) {
                  if (_isNetworkError(snapshot.error)) {
                    return AppNetworkErrorView(
                      ...
                      description: '네트워크 연결 후 공부 기록을 다시 불러와 주세요.',
                      retryButtonText: '다시 시도',
                      onRetryPressed: _reloadRecords,
                    );
                  }

                  return AppErrorView(
                    message: '공부 기록을 불러오지 못했습니다.',
                ...
                allRecordList = snapshot.data?.docs.toList() ?? [];

                List<QueryDocumentSnapshot<Map<String, dynamic>>>
                visibleRecordList = _filterRecordList(allRecordList);

                if (visibleRecordList.isEmpty && _selectedPeriod != '이번 달') {
                  return _buildEmptyPeriodView();
                }

                return _buildRecordContent(allRecordList, visibleRecordList);
              },
            ),
    );
  }
