...
class _StudyTimerPageState extends State<StudyTimerPage>
  ...
  String _formatCompactStudySeconds(
      int totalSeconds,
      ) {
    if (totalSeconds <= 0) {
      return '0분';
  ...
  int _getStoredTotalStudySeconds(
    ...
    dynamic secondsValue =
    memberData['totalStudySeconds'];
  ...
  int _getPomodoroRemainingSeconds() {
    int remainingSeconds =
        _getCurrentPhaseTargetSeconds() -
            _getCurrentPhaseSeconds();
  ...
  Future<void> _migrateOldTotalStudyTime(
      String uid,
      ) async {
    ...
    try {
      ...
      await memberDocument.set({
        ...
        exactTotalSeconds,
        'totalStudyMinutes':
        exactTotalSeconds ~/ 60,
        'updatedAt':
        FieldValue.serverTimestamp(),
      }, SetOptions(merge: true));
    } catch (error) {
      debugPrint(
        '기존 누적 공부시간 변환 오류: $error',
      );
    }
  }

  Future<void> _ensureDefaultSubjects(
      String uid,
      ) async {
  ...
  Future<bool> _checkMember() async {
    ...
    if (memberSnapshot.exists) {
      ...
      if (status == 'PENDING') {
        _showMessage(
          '참여 승인 후 공부시간을 기록할 수 있습니다.',
        );
        return false;
      }

      _showMessage(
        '현재 참여 중인 그룹원만 기록할 수 있습니다.',
      );
      return false;
    }
  ...
  Future<void> _writeLiveStatus() async {
    ...
    try {
      await memberDocument.set({
        ...
        'timerPhaseSeconds':
        _phaseSeconds,
        'timerSegmentStartedAt':
        _isSessionActive &&
            !_isPaused
            ? FieldValue.serverTimestamp()
            : null,
        'pomodoroStudyMinutes':
        _pomodoroStudyMinutes,
        'pomodoroRestMinutes':
        _pomodoroRestMinutes,
        'focusExitCount':
        _focusExitCount,
        'pauseCount':
        _pauseCount,
  ...
  Future<void> _saveFocusExitCount() async {
    ...
    try {
      ...
      debugPrint(
        '집중 이탈 횟수 저장 오류: $error',
      );
    }
  }

  void _restoreLiveStudy(
      Map<String, dynamic> memberData,
      ) {
    ...
    bool hasNewTimerSession =
        memberData['timerSessionActive'] ==
            true;

    bool hasOldTimerSession =
        memberData['isStudying'] == true;

    if (!hasNewTimerSession &&
        !hasOldTimerSession) {
      return;
    }

    String restoredMode =
        memberData['timerMode']
            ?.toString() ??
            'STOPWATCH';
    ...
    String subjectId =
        memberData['studySubjectId']
            ?.toString() ??
            '';

    String subjectName =
        memberData['studySubject']
            ?.toString() ??
            '';

    if (subjectName.isEmpty) {
      subjectName =
          memberData['currentSubject']
              ?.toString() ??
              '';
    }
    ...
    bool restoredResting =
        memberData['timerPhase']
            ?.toString() ==
            'REST';

    int restoredStudySeconds =
    _readInt(
      memberData,
      'timerStudySeconds',
    );
    ...
    int restoredPhaseSeconds =
    _readInt(
      memberData,
      'timerPhaseSeconds',
    );

    int restoredExitCount =
    _readInt(
      memberData,
      'focusExitCount',
    );

    int restoredPauseCount =
    _readInt(
      memberData,
      'pauseCount',
    );

    int restoredFocusRounds =
    _readInt(
      memberData,
      'completedFocusRounds',
    );

    int restoredStudyMinutes =
    _readInt(
      memberData,
      'pomodoroStudyMinutes',
      fallback: 25,
    ...
    is Timestamp) {
      restoredSessionStartedAt =
          sessionStartedAtValue
              .toDate()
              .toLocal();
    }

    dynamic segmentStartedAtValue =
    memberData[
    'timerSegmentStartedAt'];

    DateTime? restoredSegmentStartedAt;

    if (segmentStartedAtValue
    is Timestamp) {
      restoredSegmentStartedAt =
          segmentStartedAtValue
              .toDate()
              .toLocal();
    }
    ...
    if (!hasNewTimerSession &&
      ...
      dynamic oldStartedAtValue =
      memberData['studyStartedAt'];

      if (oldStartedAtValue
      is Timestamp) {
        restoredSegmentStartedAt =
            oldStartedAtValue
                .toDate()
    ...
    WidgetsBinding.instance
        ...
        setState(() {
          ...
          _studySeconds =
              restoredStudySeconds;

          _restSeconds =
              restoredRestSeconds;

          _phaseSeconds =
              restoredPhaseSeconds;
          ...
          _pomodoroStudyMinutes =
          restoredStudyMinutes < 1
              ? 25
              : restoredStudyMinutes;

          _pomodoroRestMinutes =
          restoredRestMinutes < 1
              ? 5
              : restoredRestMinutes;
  ...
  Future<void> _stopStudy() async {
    ...
    try {
      ...
      _showMessage(
        '공부시간을 저장하지 못했습니다. 타이머는 계속 유지됩니다.',
      );
    }
  }
  ...
  Future<void> _showPomodoroSettings() async {
    ...
    setState(() {
      _pomodoroStudyMinutes =
          result['studyMinutes'] ?? 25;

      _pomodoroRestMinutes =
          result['restMinutes'] ?? 5;
    });
  }

  Map<String, int>
  _createTodaySubjectSecondsMap(
      QuerySnapshot<Map<String, dynamic>>
      recordSnapshot,
      String uid,
      ) {
  ...
  int _getTodayTotalSeconds(
    ...
    todaySubjectSecondsMap.forEach(
          (key, value) {
        totalSeconds += value;
      },
    );
...
class _StudySubjectDialog
  ...
  @override
  State<_StudySubjectDialog>
  createState() {
    return _StudySubjectDialogState();
  }
}

class _StudySubjectDialogState
    extends State<_StudySubjectDialog> {
...
class _PomodoroSettingsDialog
  ...
  @override
  State<_PomodoroSettingsDialog>
  createState() {
    return _PomodoroSettingsDialogState();
  }
}

class _PomodoroSettingsDialogState
    extends State<_PomodoroSettingsDialog> {
  ...
  void initState() {
    ...
    _studyMinutes =
        widget.initialStudyMinutes;

    _restMinutes =
        widget.initialRestMinutes;
  }
